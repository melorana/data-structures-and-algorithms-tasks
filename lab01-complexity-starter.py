...
def check_variant(data_dir: Path, variant: 19) -> None:
...
def self_check() -> None:
    ...
    def count_equal_pairs_reference(a: list[int]) -> int:
        counts = {}
        for x in a:
            if x in counts:
                counts[x] += 1
    ...
    # Инвариант 1: если все элементы массива попарно различны, совпадающих пар быть не может ни при каком n
    # это верно всегда, а не только для конкретного примера.
    assert count_equal_pairs(list(range(50))) == 0

    # Инвариант 2: количество совпадающих пар - свойство мультимножества значений, а не порядка их следования.
    # Перестановка элементов не должна менять результат.
    rng2 = random.Random(1)
    a = [rng2.randint(0, 9) for _ in range(40)]
    shuffled = a[:]
    rng2.shuffle(shuffled)
    assert count_equal_pairs(a) == count_equal_pairs(shuffled)

    # Инвариант 3: массовая сверка с независимой формулой на случайных массивах с узким диапозоном значений (много повторов)
    # именно там легче всего ошибиться на единицу в границах двойного цикла.
    for _ in range(200):
        a = [rng2.randint(0, 9) for _ in range(rng2.randint(1, 60))]
        assert count_equal_pairs(a) == count_equal_pairs_reference(a)
